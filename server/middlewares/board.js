 ...
 */
const canSeeViaSocket = async (socket, next) => {
    socket.on('subscribeToBoard', async (boardId) => {
        console.log('inside middleware', boardId);
        next();
    });
    next();
    ...
    try {
        ...
        */
        ...
        if (e.name === 'CastError') {
            next(new Error({ error: 'Board not found' }));
        } if (e.status) {
            next(new Error({ error: e.message }));
        }
        return next(new Error({ error: 'Internal server error' }));
    }
};
